...
class QuotedMess:
    ...
    def __post_init__(self):
        ...
        elif self.message.HasField("groupStatusMessageV2"):
            self.message = self.message.groupStatusMessageV2.message
    ...
    def media_type(self) -> Optional[str]:
        ...
        if not msg_fields:  
            return None
        return msg_fields[0][0].name.replace("Message", "")
        
    async def react(self, text: str):
        if self._chat.Server == "newsletter":
            server_id = getattr(self.context_info, "newsletterServerId", 0)
            await self.client.newsletter_send_reaction(self._chat, server_id, text, self.id)
        else:
    ...
    def mentioned_jid(self) -> List[str]:
        ...
        _, field_value = msg_fields[0]
        if field_name := msg_fields[0][0].name in ("stickerMessage", "locationMessage"):
            return []

        if not hasattr(field_value, "contextInfo"):
            return []

        mentioned_jid_list = getattr(field_value.contextInfo, "mentionedJID", [])
...
class Mess:
    ...
    async def react(self, text: str):
        if self.chat.Server == "newsletter":
            server_id = getattr(self.info, "NewsletterServerID", 0)
            await self.client.newsletter_send_reaction(self.chat, server_id, text, self.id)
        else:
            await self.client.send_message(
                self.chat,
                await self.client.build_reaction(
    ...
    @property
    def _real_msg(self) -> RawMessage:
        msg = self.message.Message
        if msg.HasField("viewOnceMessage"):
            return msg.viewOnceMessage.message
        if msg.HasField("viewOnceMessageV2"):
            return msg.viewOnceMessageV2.message
        if msg.HasField("groupStatusMessageV2"):
            return msg.groupStatusMessageV2.message
        return msg

    @cached_property
    def is_media(self) -> bool:
        msg_fields = self._real_msg.ListFields()
        if not msg_fields:
            return False
        return msg_fields[0][0].name in (
            "imageMessage", "videoMessage", "audioMessage",
    ...
    def text(self) -> str:
        ...
        if msg.HasField("interactiveResponseMessage"):
            try:
                params = json.loads(msg.interactiveResponseMessage.nativeFlowResponseMessage.paramsJSON)
                return params.get("id", "")
            except:
                return msg.interactiveResponseMessage.nativeFlowResponseMessage.paramsJSON

        # Handle Template Response
        if msg.HasField("templateButtonReplyMessage"):
            return msg.templateButtonReplyMessage.selectedID

        # Handle List Response
        if msg.HasField("listResponseMessage"):
            return msg.listResponseMessage.singleSelectReply.selectedRowID

        # Handle Buttons Response
        if msg.HasField("buttonsResponseMessage"):
            return msg.buttonsResponseMessage.selectedButtonID

        for _, field_value in msg.ListFields():
            if isinstance(field_value, str):
                return field_value
            for attr in ("text", "caption", "name", "conversation"):
    ...
    async def download(self):
        return await self.client.download_any(self._real_msg)
    ...
    def mentioned_jid(self) -> List[str]:
        ...
        _, field_value = msg_fields[0]
        if msg_fields[0][0].name in ("stickerMessage", "locationMessage"):
            return []
